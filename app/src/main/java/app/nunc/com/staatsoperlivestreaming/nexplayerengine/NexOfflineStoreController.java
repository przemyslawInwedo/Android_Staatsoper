...
public class NexOfflineStoreController {
	...
	public int startOfflineStore(String url, String storedInfoFilePath, int transportType) {
		...
		return startOfflineStore(mContext, mNexPlayer, mOfflineStoreSettingData, transportType);
	}
	...
	public int startOfflineStore(FileDescriptor storedInfoFD, int transportType) {
		...
		if(!TextUtils.isEmpty(mOfflineStoreSettingData.storeURL)) {
			ret = startOfflineStore(mContext, mNexPlayer, mOfflineStoreSettingData, transportType);
		}
		return ret;
	}
	...
	public int startOfflineStore(String storedInfoFilePath, int transportType) {
		...
		return startOfflineStore(mOfflineStoreSettingData.storeURL, storedInfoFilePath, transportType);
	}
	...
	private int startOfflineStore(Context context, NexPlayer player, NexSettingDataForStoring settings, int transportType) {
		...
		if( shouldOpenPlayer ) {
			...
			if( player != null && player.isInitialized() ) {
				ret = setupPlayerBeforeOpen(context, player, settings);
	...
	private int setupPlayerBeforeOpen(Context context, NexPlayer player, final NexSettingDataForStoring settings) {
		int ret = NexPlayer.NexErrorCode.NONE.getIntegerCode();
		...
		if(checkWidevineDrmIntersection(2, settings.drmType)) {
			...
			if(mNexWVDRM.initDRMManager(enginePath, strCertPath, keyServer, offlineMode) == 0) {
