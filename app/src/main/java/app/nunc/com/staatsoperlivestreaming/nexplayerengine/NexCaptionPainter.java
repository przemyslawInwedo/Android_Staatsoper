...
import static app.nunc.com.staatsoperlivestreaming.nexplayerengine.NexCaptionSetting.*;
import static app.nunc.com.staatsoperlivestreaming.nexplayerengine.NexCaptionSetting.DEFAULT;
import static app.nunc.com.staatsoperlivestreaming.nexplayerengine.NexClosedCaption.convertEncodingType;
...
public class NexCaptionPainter extends ViewGroup {
    ...
    private void updateDrawingView(NexCaptionRenderingAttribute renderingAttribute) {
        ...
        if (hasDrawingData) {
            renderingAttribute.view = makeView(renderingAttribute);
    ...
    private int getBaseTextColor(ArrayList<NodeString> nodeStrings) {
        ...
        if (null != nodeStrings && !nodeStrings.isEmpty()) {
            baseTextColor = nodeStrings.get(0).mFontColor;
        }
    ...
    private int getBaseBackgroundColor(ArrayList<NodeString> nodeStrings) {
        ...
        if (null != nodeStrings && !nodeStrings.isEmpty()) {
            baseBackgroundColor = nodeStrings.get(0).mBackgroundColor;
        }
    ...
    private View makeView(NexCaptionRenderingAttribute renderingAttribute) {
        View view;

        if (null != renderingAttribute.mImage) {
            view = makeImageView(renderingAttribute);
        } else {
            view = makeCaptionView(renderingAttribute);
        }
    ...
    private Rect getCaptionPosition(NexCaptionWindowRect relativeRect, View view) {
        ...
        return mCaptionExtractor.getCaptionPosition(relativeRect, view.getMeasuredWidth(), view.getMeasuredHeight());
    }
    ...
    private void adjustViewRect(TextView view, Rect winRect) {
        ...
        NexLog.d(LOG_TAG, "adjustViewRect line count : " + lineCount + " , height : " + height + " , originHeight : " + originHeight);
