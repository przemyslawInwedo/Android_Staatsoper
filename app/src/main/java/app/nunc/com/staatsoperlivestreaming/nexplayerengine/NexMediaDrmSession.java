...
import static app.nunc.com.staatsoperlivestreaming.nexplayerengine.NexMediaDrm.TIME_UNSET;
import static app.nunc.com.staatsoperlivestreaming.nexplayerengine.NexSystemInfo.NEX_SUPPORT_PLATFORM_OREO;
...
public class NexMediaDrmSession {
    ...
    NexMediaDrmSession(UUID uuid, NexMediaDrm mediaDrm,
                       ...
                       int initialDrmRequestRetryCount, int mediaType) {
        this.uuid = uuid;
        ...
        if (offlineLicenseKeySetId == null) {
            if (isPSSHContained(initData)) {
                byte[] newPssh = new byte[initData.length - PSSH_SIZE];
                int index = 0;

                for (int i = initData.length - (initData.length - PSSH_SIZE); i < initData.length; i++) {
    ...
    private void onKeyResponse(Object response) {
        ...
        try {
            ...
            if (mode == NexMediaDrmSessionManager.MODE_RELEASE) {
                ...
                state = STATE_OPENED_WITH_KEYS;
                if (eventListener != null) {
                    eventListener.onDrmKeysLoaded(mediaType, keySetId, sessionId);
                }
            }
        } catch (Exception e) {
            onKeysError(e);
...
final class ClearKeyUtil {
    ...
    static byte[] adjustRequestData(byte[] request) {
        if (NexSystemInfo.getPlatformInfo() > NEX_SUPPORT_PLATFORM_OREO) {
            return request;
        }
