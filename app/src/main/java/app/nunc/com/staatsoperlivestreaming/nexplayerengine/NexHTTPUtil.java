...
public class NexHTTPUtil {
  ...
  public static byte[] executePost(String url, byte[] data, Map<String, String> requestProperties)
    ...
    try {
      ...
      if (data != null) {
        Log.e(TAG, "create BufferedOutputStream.  data:" + data.length);//.toString());
        OutputStream out = new BufferedOutputStream(urlConnection.getOutputStream());
        NexLog.d(TAG, "create BufferedOutputStream. Done");
        try {
          out.write(data);
        } finally {
          Log.e(TAG, "write output buffer..");
          out.close();
        }
      }
  ...
  private static byte[] convertInputStreamToByteArray(InputStream inputStream) throws IOException {
    ...
    while ((count = inputStream.read(data)) != -1) {
      ...
      NexLog.d(TAG, "convertInputStreamToByteArray. Read:" + count + "  write:" + bos.size());
    }
    bos.flush();
    bos.close();
    inputStream.close();
