...
public final class NexPlayer
{
    ...
    private static final int NEXPLAYER_VERSION_MINOR					= 63;
     ...
     * you can retrieve them using the {@link app.nexstreaming.nexplayerengine.NexProperty#getPropertyCode() getPropertyCode}
     * method of a member of this enum, and the methods {@link app.nexstreaming.nexplayerengine.NexPlayer#getProperties(int) getProperties(int)} and
     * {@link app.nexstreaming.nexplayerengine.NexPlayer#setProperties(int, int) setProperties(int, int)} can be used to get or set a property based
    ...
    public enum NexProperty {
        ...
        /**
         * Avoids waiting for the first WebVTT segment to download when starting to play content.
         *
        ...
        SET_APPLS_PROGRAM_DATE_TIME_PRESENTATION_DELAY (590),
     ...
     *
     * To get the error constant for a given code, call {@link app.nexstreaming.nexplayerengine.NexPlayer.NexErrorCode#fromIntegerValue(int) fromIntegerValue(int)}.
     *
     * To get the error code given an error constant, call {@link app.nexstreaming.nexplayerengine.NexPlayer.NexErrorCode#getIntegerCode() getIntegerCode()}.
     *
	...
	ERROR_INVALID_SERVER_STATUSCODE(        0x00020000,NexErrorCategory.PROTOCOL, "The HTTP response data recevied from the Server is error."),
        ...
        public String getDesc() {
            ...
            if (mCode == NexErrorCode.UNKNOWN.getIntegerCode())
                Desc = mDesc + " - Error Code : 0x" + String.format("%08X", mSubCode) + ".";
            else
                Desc = mDesc;
            return Desc;
        }
        ...
        public static int getUnknownSubCode( ) {
            NexLog.d(TAG, "Unknown Error. Error Code = 0x" + String.format("%08X%n", mSubCode));
            return mSubCode;
        }
    }
     ...
     */
    public boolean init( Context context, int logLevel) {
        ...
        if( !mNexPlayerInit ) {
            ...
            if( nRet == 0 ) {
                ...
                NexLog.d( TAG, "Init failure: " + nRet );
            }
        }
    ...
    private int setupPlayerBeforeOpen(Context context, NexPlayer player, NexSettingDataForStoring settings) {
        ...
        if( !TextUtils.isEmpty(settings.mediaDrmKeyServer) ) {
            ...
            if(2 == (2 & settings.drmType)) {
                ...
                String enginePath = context.getApplicationInfo().dataDir + "/lib/" + engine;

                if (mNexWVDRM.initDRMManager(enginePath, strCertPath, settings.mediaDrmKeyServer, offlineMode) == 0) {
     ...
     *
     * For example: \code addHTTPHeaderFields("Cookie: Cookie test value."); \endcode
     *
     ...
     *            The application package name. (ex. app.nexstreaming.nexplayersample)
     * @param sdkInfo
        ...
        if( mVideoRendererListener != null) {
            switch(event.what) {
                case NexPlayerEvent.NEXPLAYER_EVENT_VIDEO_RENDER_PREPARED:
                    ...
                    mEventForwarder.handleEvent(nexplayer, event);
                    break;
                                       ...
                                       int arg2, int arg3, int arg4, Object obj )
    {
        ...
        nRet = (Integer)mEventForwarder.handleEvent(nexplayer, mListener, event);
        return nRet;
    }
    ...
    private static final int PROPERTY_MDEIA_DRM_VIDEO_SESSION = 0x90002;
    private static final int PROPERTY_MDEIA_DRM_AUDIO_SESSION = 0x90003;
    ...
    @TargetApi(18)
    private int callbackFromNativeMediaDrm(Object nexplayer_ref, int what, byte[] uuid, byte[] pssh, int arg1, int arg2, Object obj)
    {
        @SuppressWarnings("unchecked") // The type cast to WeakReference<NexPlayer> is always safe because
                // this function is only called by known native code that always
                // passes an object of this type.
        ...
        if (nexplayer == null || iPlatform < NexSystemInfo.NEX_SUPPORT_PLATFORM_JELLYBEAN3) {
            return nRet;
        }

        switch (what) {
            case NexPlayerEvent.NEXPLAYER_INIT_MEDIA_DRM:
                NexLog.v(TAG, "[CB] NEXPLAYER_INIT_MEDIA_DRM. media type : " + arg2);
            ...
            case NexPlayerEvent.NEXPLAYER_DEINIT_MEDIA_DRM:
                NexLog.v(TAG, "[CB] NEXPLAYER_DEINIT_MEDIA_DRM. media type : " + arg2);
                if (null != drmSession) {
            ...
            case NexPlayerEvent.NEXPLAYER_DRM_TYPE_ACCEPTED:
                ...
                nRet = -1;
                if (null != obj) {
                    ArrayList<byte[]> systemIds = (ArrayList<byte[]>) obj;
                    for (int index = 0; index < systemIds.size(); ++index) {
                        if (selectDrmScheme(makeUUID(systemIds.get(index)))) {
                            nRet = index;
                            break;
                        }
                    }
    ...
    private int initNexMediaDrmSession(UUID uuid, byte[] pssh, OfflineMode offlineMode, int mediaType) {
        // The followings should be synced with return codes in NEXPLAYER_IDescramble.h
        final int INIT_RETURN_SUCCESS            = 0x00000000;
        final int INIT_RETURN_ERROR              = 0x00000002;
        final int INIT_RETURN_WILLSET_SESSIONID  = 0x00000003;

        int nRet = INIT_RETURN_ERROR;
        ...
        try {
            ...
            NexMediaDrmSession session = drmSessionManager.acquireSession(drmHandlerThread.getLooper(), pssh, offlineMode, mediaType);
            ...
            if (null != session) {
                ...
                switch (session.getState()) {
                    ...
                    case NexMediaDrmSession.STATE_ERROR:
                        nRet = INIT_RETURN_ERROR;
                        break;
                    case NexMediaDrmSession.STATE_OPENING:
                        nRet = INIT_RETURN_WILLSET_SESSIONID;
                        break;
                    case NexMediaDrmSession.STATE_OPENED:
                    case NexMediaDrmSession.STATE_OPENED_WITH_KEYS:
                        int property = 0;
                        if (mediaType == 0) {
                            property = PROPERTY_MDEIA_DRM_VIDEO_SESSION;
                        } else if (mediaType == 2) {
                            property = PROPERTY_MDEIA_DRM_AUDIO_SESSION;
                        }

                        setProperties(property, session.getSessionId());
                        nRet = INIT_RETURN_SUCCESS;
                        break;
                }
            }
        } catch (Exception e) {
    ...
    private NexMediaDrmSessionManager buildMediaDrmSessionManager(UUID uuid) throws Exception {
        ...
        eventDrmSessionListener = new NexMediaDrmSessionManager.EventListener() {
            @Override
            public void onDrmKeysLoaded(int mediaType, byte[] keySetId, byte[] sessionId) {
                int property = 0;
                if (mediaType == 0) {
                    property = PROPERTY_MDEIA_DRM_VIDEO_SESSION;
                } else if (mediaType == 2) {
                ...
                if (NexPlayer.OfflineMode.STORE == offlineMode || NexPlayer.OfflineMode.RETRIEVE_STORE == offlineMode) {
                ...
                NexLog.v(TAG, "onDrmKeysLoaded set property");
                setProperties(property, sessionId);
            }
    ...
    private boolean selectDrmScheme(UUID drmScheme) {
        boolean select = false;

        if (canUseDrmScheme(drmScheme)) {
            select = true;
        }
        return select;
    }
    ...
    private boolean canUseDrmScheme(UUID drmScheme) {
        ...
        if (0 == drmScheme.compareTo(NexMediaDrm.WIDEVINE_UUID)) {
            NexLog.e(TAG, "WideVine DRM (" + NexMediaDrm.WIDEVINE_UUID.toString() + ")");
        } else if (0 == drmScheme.compareTo(NexMediaDrm.PLAYREADY_UUID)) {
            NexLog.e(TAG, "PlayReady DRM (" + NexMediaDrm.WIDEVINE_UUID.toString() + ")");
        } else {
            NexLog.e(TAG, "Unknown DRM (" +drmScheme.toString() + ")");
        }

        if(NexMediaDrm.isSupportDRMScheme(drmScheme)) {
            NexLog.e(TAG, "This device support this DRM Scheme");
            support = true;
            NexLog.e(TAG, "This is supported drm (" + NexMediaDrm.WIDEVINE_UUID.toString() + ")");
        } else {
    ...
    interface IMetaDataEventListener {
        /**
         * \brief  This method reports the Emag information that is contained in the emsg box.
         *
         * @param mp    The NexPlayer&trade;&nbsp;object to which this event applies.
