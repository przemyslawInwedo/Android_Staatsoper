...
public class NexPlayerPrefs extends PreferenceActivity implements OnSharedPreferenceChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		PreferenceScreen AutoLowLatencyScreen = getPreferenceScreen();
		if (AutoLowLatencyScreen != null) {
			boolean value = AutoLowLatencyScreen.getSharedPreferences().getBoolean("Automatic Low-Latency Enable", false);
			if (value) {
				Log.d(LOG_TAG, "disable setup low-latency value slider");
				getPreferenceScreen().findPreference("lowlatencyValue").setEnabled(false);
			} else {
				Log.d(LOG_TAG, "enable setup low-latency value slider");
				getPreferenceScreen().findPreference("lowlatencyValue").setEnabled(true);
			}
		}
	}
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
		...
		else if (key.equals("Automatic Low-Latency Enable")) {
			Log.d(LOG_TAG, "Automatic low latency is changed");
			PreferenceScreen AutoLowLatencyScreen = getPreferenceScreen();
			if (AutoLowLatencyScreen != null) {
				boolean value = sharedPreferences.getBoolean(key, false);
				if (value) {
