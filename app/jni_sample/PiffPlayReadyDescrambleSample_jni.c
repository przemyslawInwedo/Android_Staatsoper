...
static int PiffPlayReadyDescrambleCallbackFunc(
											...
											void*			pUserData)
{
	LOGI("[	PiffPlayReadyDescrambleCallbackFunc ] inputBuffer:%p(%lu), outputBuffer:%p, pSampleEncBox:%p(%lu), SampleIDX(%lu), TrackID(%lu), UserData:%p \n",
...
jint Java_com_nexstreaming_piffplayreadydescramblesample_PiffPlayReadyDRMManager_initDRMManager (JNIEnv * env,
																								 jobject clazz,
																								 jstring libName)
{
...
jint Java_com_nexstreaming_piffplayreadydescramblesample_PiffPlayReadyDRMManager_initDRMManagerMulti (JNIEnv * env,
														   	   jobject clazz,
														   	   ...
														   	   jstring libName)
{
    ...
    int (*fptr)(void* nexPlayerClassInstance, NEXPLAYERPiffPlayReadyDescrambleCallbackFunc pDescrambleCallbackFunc, void *pUserData); 
    ...
    /* Get DRM register function pointer*/   
    fptr = (int (*)(void* nexPlayerClassInstance, NEXPLAYERPiffPlayReadyDescrambleCallbackFunc pDescrambleCallbackFunc, void *pUserData))dlsym(handle, NEXPLAYERENGINE_PIFFPLAYREADY_DESCRAMBLE_CALLBACK_FUNC);
    ...
    /* Register DRM descramble function */
    if (fptr != NULL)
        (*fptr)((void*)nexPlayerInstance, PiffPlayReadyDescrambleCallbackFunc, NULL);		
